...
#    if TRACY_CUDA_ENABLE_POWER_MONITORING
        struct PowerMonitor
        {
            static constexpr const char* PlotName = "GPU Power (mW)";

            std::atomic<bool> running = false;
            uint32_t samplingPeriodMs = 1; // Configurable sampling rate
            std::thread thread;
            ...
            void Start()
            {
                ...
                {
                    fprintf( stderr, "Tracy CUDA: NVML init failed: %s\n", nvmlErrorString( result ) );
                    running = false;
                    return;
                }
                ...
                if( result != NVML_SUCCESS )
                {
                    fprintf( stderr, "Tracy CUDA: Failed to get GPU handle: %s\n", nvmlErrorString( result ) );
                    nvmlShutdown();
                    running = false;
                    return;
                }

                // Configure the Tracy plot
                TracyPlotConfig( PlotName, tracy::PlotFormatType::Number, false, true, 0 );

                // Start monitoring thread
                thread = std::thread(
                    [this]()
                    {
                        tracy::SetThreadName( "Tracy GPU Power Monitor" );

                        unsigned int powerMw = 0;
                        while( running.load( std::memory_order_relaxed ) )
                        {
                            nvmlReturn_t result = nvmlDeviceGetPowerUsage( device, &powerMw );
                            if( result == NVML_SUCCESS )
                            {
                                TracyPlot( PlotName, static_cast<int64_t>( powerMw ) );
                            }

                            std::this_thread::sleep_for( std::chrono::milliseconds( samplingPeriodMs ) );
                        }
                    } );
            }
