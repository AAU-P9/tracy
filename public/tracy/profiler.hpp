...
{
    // Tracy message for total ranges
    std::string totalRangesMsg = "Total Profiler Ranges: " + std::to_string(m_profilerRanges.size());
    TracyMessage(totalRangesMsg.c_str(), totalRangesMsg.size());

    for (size_t rangeIndex = 0; rangeIndex < m_profilerRanges.size(); ++rangeIndex)
    {
        const auto &profilerRange = m_profilerRanges[rangeIndex];

        // Tracy message for each range
        std::string rangeMsg = "Profiler Range [" + std::to_string(rangeIndex) + "]: " + profilerRange.rangeName;
        TracyMessage(rangeMsg.c_str(), rangeMsg.size());

        for (const auto &metric : profilerRange.metricValues)
        {
            // Tracy message for each metric
            std::string metricMsg = profilerRange.rangeName + " - " + metric.first + ": " + std::to_string(metric.second);
            TracyMessage(metricMsg.c_str(), metricMsg.size());
        }
    }
}
...
inline RangeProfilerTarget::RangeProfilerTarget(CUcontext ctx, RangeProfilerConfig &config) : m_context(ctx),
                                                                                              isProfilingActive(0),
                                                                                              mConfig(config)
{
    m_context = ctx;
